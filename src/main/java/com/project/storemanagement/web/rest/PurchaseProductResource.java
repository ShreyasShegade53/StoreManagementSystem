package com.project.storemanagement.web.rest;

import com.project.storemanagement.domain.PurchaseProduct;
import com.project.storemanagement.repository.PurchaseProductRepository;
import com.project.storemanagement.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.project.storemanagement.domain.PurchaseProduct}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PurchaseProductResource {
    ...
    public ResponseEntity<PurchaseProduct> updatePurchaseProduct(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, purchaseProduct.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<PurchaseProduct> partialUpdatePurchaseProduct(
        ...
        Optional<PurchaseProduct> result = purchaseProductRepository
            ...
            .map(existingPurchaseProduct -> {
                if (purchaseProduct.getName() != null) {
                    existingPurchaseProduct.setName(purchaseProduct.getName());
                }
                if (purchaseProduct.getCategory() != null) {
                    existingPurchaseProduct.setCategory(purchaseProduct.getCategory());
                }
                if (purchaseProduct.getPrice() != null) {
                    existingPurchaseProduct.setPrice(purchaseProduct.getPrice());
                }
                if (purchaseProduct.getUnits() != null) {
                    existingPurchaseProduct.setUnits(purchaseProduct.getUnits());
                }
                if (purchaseProduct.getFrom() != null) {
                    existingPurchaseProduct.setFrom(purchaseProduct.getFrom());
                }
                if (purchaseProduct.getCompany() != null) {
                    existingPurchaseProduct.setCompany(purchaseProduct.getCompany());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, purchaseProduct.getId().toString())
        );
    }
